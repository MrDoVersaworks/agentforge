...
router.post('/register', authRateLimiter, validate(registerSchema), asyncHandler(async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(201).json({ success: true, data: { accessToken: result.accessToken, user: result.user, csrfToken } });
  } catch (error: unknown) {
    if (error instanceof Error && error.message.includes('already exists')) {
      res.status(400).json({ success: false, error: { code: 'ERR_USER_EXISTS', message: error.message } });
      return;
    }
    throw error;
  }
...
router.post('/login', authRateLimiter, validate(loginSchema), asyncHandler(async (req: Request, res: Response): Promise<void> => {
  try {
    const body = req.body as { email: string; password: string };
    const result = await loginUser(body.email, body.password);
    const csrfToken = setCsrfCookie(res);
    res.cookie(REFRESH_COOKIE_NAME, result.refreshToken, authCookieOptions());
    res.status(200).json({ success: true, data: { accessToken: result.accessToken, user: result.user, csrfToken } });
  } catch (error: unknown) {
    if (error instanceof Error && error.message.includes('Invalid email or password')) {
      res.status(401).json({ success: false, error: { code: 'ERR_INVALID_CREDENTIALS', message: error.message } });
      return;
    }
    throw error;
  }
...
router.post('/refresh', asyncHandler(async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const refreshToken = req.cookies[REFRESH_COOKIE_NAME] as string | undefined;
    if (!refreshToken) {
      res.status(401).json({ success: false, error: { code: 'ERR_NO_REFRESH_TOKEN', message: 'No refresh token provided.' } });
      return;
    }
    const result = await refreshAccessToken(refreshToken);
    const csrfToken = setCsrfCookie(res);
    res.cookie(REFRESH_COOKIE_NAME, result.refreshToken, authCookieOptions());
    res.status(200).json({ success: true, data: { accessToken: result.accessToken, csrfToken } });
  } catch {
    res.clearCookie(REFRESH_COOKIE_NAME, { path: '/' });
    res.clearCookie(CSRF_COOKIE_NAME, { path: '/' });
    res.status(401).json({ success: false, error: { code: 'ERR_REFRESH_EXPIRED', message: 'Invalid or expired refresh token. Please login again.' } });
  }
}));
...
router.delete('/account', authMiddleware, validate(deleteAccountSchema), asyncHandler(async (req: Request, res: Response): Promise<void> => {
  try {
    const userId = req.user?.id as string;
    ...
    if (error instanceof Error && error.message.includes('password')) {
      res.status(403).json({ success: false, error: { code: 'ERR_INVALID_PASSWORD', message: error.message } });
      return;
    }
    throw error;
  }
